...
func init() {
	...
	logger = zerolog.New(logout).
		...
		With().Caller().Logger().
		With().Str("role", "status_ack.go").Logger()
}
...
func (n *node) waitForAck(rumorPkt transport.Packet, neighbor string) {
	...
	logger.Warn().Msgf("Added channel for ack %s", packetID)
	...
	for {
		select {
		case <-n.stopCh:
			logger.Info().Msg("Stopping waitForAck")

			return

		case ackMsg := <-ackReceived:
			if ackMsg.AckedPacketID == packetID {
				logger.Info().Msgf("Ack received for PacketID %s", packetID)
		...
		case <-time.After(ackTimeout):
			...
			logger.Warn().Msgf("Ack timeout for PacketID %s", packetID)
			n.ackChannel.Remove(packetID)
			close(ackReceived)
			//get other neighbor
			...
			if len(otherNeighbors) == 0 {
				logger.Warn().Msg("No other neighbors to resend rumor")
			}
			...
			logger.Debug().Msgf("Sending rumor to %s", randNeighbor)
...
func (n *node) compareViews(view types.StatusMessage) (uint, map[string][]types.Rumor) {
	...
	for origin, remoteSeq := range view {
		...
		// logger.Info().Msgf("localSeq: %d", localSeq)
		// logger.Info().Msgf("localexists: %t", localExists)
...
func (n *node) sendStatusMessage(destination string) error {
	...
	logger.Info().Msgf("Status message sent to %s", destination)
