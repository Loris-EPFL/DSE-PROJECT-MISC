...
func init() {
	...
	logger = zerolog.New(logout).
		...
		With().Caller().Logger().
		With().Str("role", "data_sharing.go").Logger()
}
...
func (n *node) Upload(data io.Reader) (metahash string, err error) {
	...
	if chunkSize <= 0 {
		logger.Error().Msg("Chunk size <= than 0")
	...
	logger.Debug().Msgf("Read %s", dataBytes)
	if err != nil {
		logger.Error().Msg("Error reading data")
...
func (n *node) Download(metahash string) ([]byte, error) {
	...
	if metafileContent == nil {
		...
		logger.Warn().Msgf("Metafile content after fetching it: %s", metafileContent)
		logger.Warn().Msgf("Metahash: %s", metahash)

		if err != nil {
			logger.Error().Msgf("Failed to fetch metafile %s: %v", metahash, err)
	...
	for i, chunkHash := range chunkHashes {
		...
		if chunkData == nil {
			...
			if err != nil {
				logger.Error().Msgf("Failed to fetch chunk %s: %v", chunkHash, err)
...
func (n *node) Tag(name string, metahash string) error {
	...
	logger.Debug().Msg("No other peers, setting the metahash directly")
...
func (n *node) Resolve(name string) string {
	...
	if name == "" {
		logger.Error().Msg("Invalid name to be searched in the naming storage")
		return ""
	}
	storage := n.conf.Storage.GetNamingStore()
...
func (n *node) sendDataRequest(key string, destPeer string) ([]byte, error) {
	...
	if err != nil {
		logger.Error().Msg("Error marshaling the data request message")
		return nil, err
	}
	...
	for retries := 0; retries < maxRetries; retries++ {
		...
		if err != nil {
			logger.Error().Msgf("Error sending request, got: %s", err)
			return nil, err
		}
		select {
		case dataReply := <-replyChan:
			...
			if dataReply.RequestID == requestID {
				if dataReply.Value != nil {
					logger.Warn().Msgf("Closing channel with request ID: %s", requestID)
...
func (n *node) fetchWithBackOff(key string) ([]byte, error) {
	...
	logger.Info().Msg("I am here")

	peers := n.catalog.GetPeersForKey(key)
	logger.Debug().Msgf("Peers: %s and peers length: %d", peers, len(peers))
	...
	logger.Warn().Msgf("Sending data request from %s to %s", n.conf.Socket.GetAddress(), selectedPeer)
	...
	if err == nil {
		...
		if computedHash != expectedHash {
			...
			logger.Info().Msgf("Data received don't match expected data for key %s", key)
...
func (n *node) divideBudget(totalBudget, numNeighbors uint64) []uint64 {
	...
	if totalBudget == 0 || numNeighbors == 0 {
		logger.Error().Msg("Budget or neighbors not sufficient")
		return budgets
	}
...
func (n *node) sendSearchRequest(requestID string, budget uint64, destPeer string, reg regexp.Regexp) {
	...
	if err != nil {
		logger.Error().Msgf("Unable to marshal message, got %s", err)
		return
	}
	...
	if !exists {
		logger.Error().Msgf("No way to send message to %s from %s", destPeer, n.conf.Socket.GetAddress())
		return
	}
	...
	if err != nil {
		logger.Error().Msgf("Error while trying to send search request to %s Got: %v", destPeer, err)
	}
}
