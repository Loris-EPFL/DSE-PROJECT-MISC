...
func (n *node) sendMissingRumors(source string, missing map[string][]types.Rumor) error {
	...
	if err != nil {
		logger.Error().Msgf("Failed to marshal RumorsMessage")
	...
	if err != nil {
		logger.Error().Msgf("Failed to send RumorsMessage to %s", source)
...
func (n *node) forwardRumorsMessage(rumors *types.RumorsMessage, receivedPkt transport.Packet) error {
	...
	if err != nil {
		logger.Error().Msgf("Failed to marshal %s: %v", rumors, err)
		return err
	}
	...
	if err != nil {
		logger.Error().Msgf("Failed to forward packet %s to %s: %v", fwPkt, randNeighbor, err)
		return err
	}
